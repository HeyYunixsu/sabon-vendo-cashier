...
## 14. Product Photography Integration (superseded — see §15)

Kept for history. This approach (real product photos as card backgrounds) has been replaced by icon-based tiles per §15, mainly because square source photos vs. landscape/any-size cards kept fighting each other across screen sizes. Not in use.
...
| `slot5.png` | 5 | Bleach Original | Cream ceramic jug |
| `slot6.png` | 6 | Bleach Colored | Green ceramic jug |
...
- Product photos are **not** currently shipped: the dashboard renders inline SVG
  icons. `public/images/` was removed as unused.
...
- **Slot 6 open question:** you'd earlier flagged Bleach Colored as physically unwired/unavailable, so slot 6 keeps the existing `.inactive` treatment (30% opacity, non-interactive) per §9. The mockup renders it as a fully active card with its own status dot instead — I'd keep it `.inactive` (just with the new photo) unless the machine's actually been wired up since. Say so if that's changed.

---

## 15. Icon-Based Product Tiles (current approach)

Photos are out. Each product gets a flat vector icon on a colored square tile instead — like a phone's app-icon grid. This sidesteps every problem §14 ran into (1:1 source images, cropping, missing-file fallbacks, retina exports) since there's no raster asset at all — just inline SVG, which scales losslessly at any size.

### Icon + color per product
One glyph per container type (shared across the two scent variants), one accent color per product:

| Slot | Product | Icon | Accent (light mode fill / icon) |
|---|---|---|---|
| 1 | Detergent 1 | droplet | `#EEEDFE` tile / `#534AB7` icon |
| 2 | Detergent 2 | droplet | `#E6F1FB` tile / `#185FA5` icon |
| 3 | Fabcon 1 | pouch | `#E1F5EE` tile / `#0F6E56` icon |
| 4 | Fabcon 2 | pouch | `#FAECE7` tile / `#993C1D` icon |
| 5 | Bleach Original | jug | `#FAEEDA` tile / `#854F0B` icon |
| 6 | Bleach Colored | jug | `#EAF3DE` tile / `#3B6D11` icon |

Same icon per category keeps the two scent variants visually paired; the color is what tells them apart, same as the original pre-photo design already had per-product background colors.

### Icon markup (self-contained, no external font/library)
```html
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
  <!-- droplet (Detergent 1/2) -->
  <path d="M12 3c4.2 4.6 6.5 8.1 6.5 11a6.5 6.5 0 0 1-13 0c0-2.9 2.3-6.4 6.5-11z"/>
</svg>
```
```html
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
  <!-- pouch (Fabcon 1/2) -->
  <path d="M8 4h8l1 3-2 1v9a2 2 0 0 1-2 2h-2a2 2 0 0 1-2-2V8L7 7l1-3z"/>
  <path d="M10 4V2.5M14 4V2.5"/>
</svg>
```
```html
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
  <!-- jug (Bleach Original/Colored) -->
  <path d="M9 3h4v3l2 2v10a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2V8l2-2V3z"/>
  <path d="M15 9h1.5a1.5 1.5 0 0 1 1.5 1.5V15a1.5 1.5 0 0 1-1.5 1.5H15"/>
</svg>
```
`stroke="currentColor"` means the icon inherits color from its parent's CSS `color` — set that once per tile (the "icon" column in the table above) rather than editing the SVG itself. Treat these as a starting point; refine the exact paths if they don't read cleanly at small sizes.
...
.product-icon {
  width: 22%;             /* scales with the tile, not a fixed px */
  max-width: 64px;
  aspect-ratio: 1 / 1;
  border-radius: 12px;
  display: flex;
...
.product-name {
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;   /* long names truncate instead of wrapping/overflowing on narrow screens */
}
```

### Why this covers "any screen size"
- `aspect-ratio: 1/1` + a percentage width means the icon square scales with its tile at any viewport — no breakpoints needed for the icon itself.
- No raster image means no retina/DPI export step, no crop, no missing-file fallback — the SVG is inline markup, it always renders.
- Grid responsiveness lives at the layout level, independent of tile content: `grid-template-columns: repeat(auto-fit, minmax(240px, 1fr))` on the product grid lets column count adapt (1 across on a narrow POS screen, more on a wide monitor) without touching individual tiles.
- `text-overflow: ellipsis` on the product name is the one thing that needs to be explicit — without it, a long product name on a narrow tile wraps awkwardly or overflows the card.

### What carries over unchanged
`SLOT 0X` label, product name, qty number, status row, and the `padding-left` breathing-room fix from `PRODUCT_CARD_LAYOUT_FIX.md` all apply the same way here — only the photo zone is replaced by the icon tile. That layout-fix file is otherwise superseded by this section (its subject, the photo panel, no longer exists).
